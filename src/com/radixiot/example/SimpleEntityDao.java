...
public class SimpleEntityDao {
	...
	public SimpleEntityVO get(int id) {
		return this.create.select(getSelectFields()).from(table).where(table.id.eq(id)).limit(1)
	...
	public void getAll(Consumer<SimpleEntityVO> callback) {
		...
		try (Stream<Record> stream = select.stream()) {
			stream.map(this::mapRecord).filter(Objects::nonNull).forEach(callback);
		}
	}

	public void insert(SimpleEntityVO vo) {
		int id = create.insertInto(table).set(toRecord(vo)).returningResult(table.id).fetchOptional()
				.orElseThrow(NoDataFoundException::new).value1();
		vo.setId(id);
	}
